...
% L.L.M. Veldhuis (2015)
%
% Copy Right: Delft University of Technology
%
% -------------------------COMMENTS EvB----------------------------------
...
% carefull when interpreting the results
            term_in_sqrt= abs((U0*cos(aprad)+vai(i,j))^2 + (dtdr(i,j) / (factorf(i,j)*pi*rho*r(i))));
...
%Addtional plotting when required

if plotCTandEff  == 1
    data=load('outputfile.dat');
    adv=data(:,2);
    CT=data(:,5);
    efficiency=data(:,10);
